...
from config import fp_articles, fp_ref, file_data_name, article_auteurs, auteur_articles, article_ref
import time
...
class Auteur:
	...
	def cite(self, N=1):
		...
		if not int(N) > 0:
			...
			# list des auteurs influencés au rang précédant
			...
			for k in range(1, N+1):
				for auteur_courant in auteurs_rang_courant:
					for auteur in dict_aa[auteur_courant]:
						...
						tmp = data2.id_articles[self.name]
						tmp = re.split(", ",tmp[1:-1])
						for paper in tmp:
							tmp2=re.split("""', '|", "|', "|", '""", data.auteurs[int(paper)][2:-2])
							if auteur in tmp2:
