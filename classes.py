...
class Auteur:
    ...
    def cite(self, N=1):
        """
        Donne les auteurs cités par un auteur.
    ...
    def est_cite(self, N=1):
        """
        Donne les auteurs qui ont cité un auteur.
...
class Communaute():
    ...
    def afficher(self):
        ...
        # on ajoute les arrêtes créées avec le module networkx
        for edge in G.edges():
    ...
    def graph_simple(self):
        """
        Affiche un graphe Plotly via le navigateur les relations d'un auteur avec les autres auteurs pour un profondeur donnée.
        ...
        self.graph.add_weighted_edges_from([(self.auteur_central.name,membre_i,self.membres[membre_i]) for membre_i in self.membres.keys()],weight='weight')
        ...
        for k in range(self.profondeur):
            for auteur_courant in auteurs_courants:
                ...
                for auteur_i in liste_auteurs:
                    try:
                        self.graph.add_weighted_edges_from([(auteur_courant,auteur_i,self.membres[auteur_i])],weight='weight')
                    except KeyError:
                        pass
